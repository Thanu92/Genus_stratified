...
# [41] "/CO1_99_10S.fasta" "/CO1_99_1S.fasta"  "/CO1_99_2S.fasta"  "/CO1_99_3S.fasta"  "/CO1_99_4S.fasta"  "/CO1_99_5S.fasta"  "/CO1_99_6S.fasta"  "/CO1_99_7S.fasta"  "/CO1_99_8S.fasta"  "/CO1_99_9S.fasta" 
...
#Read 50 tree files
tree_files <- list.files(path="/home/thanu/Desktop/FishData/New_dataset/STRATIFIED", pattern="^[0-9]{1,2}_[0-9]{1,2}_new", full.names=TRUE, recursive=FALSE)
...
#Sister species of placement sequences in reference tree
RefTree_R_co1_20_1_List <- foreach(i=1:length(species_List[[38]])) %do% sister(ReferenceTree,species_List[[38]][i],type="terminal",label=T)
#Sister species for placement sequeneces(20%) after placing on bb tree(80% bb tree)
R_80_co1_20_1_List <- foreach(i=1:length(species_List[[38]])) %do% sister(treeList$`/80_7_new`,species_List[[38]][i],type="terminal",label=T)
...
RefTree_R_co1_40_1_List <- foreach(i=1:length(species_List[[27]])) %do% sister(ReferenceTree,species_List[[27]][i],type="terminal",label=T)
#Sister species for placement sequeneces(20%) after placing on bb tree(80% bb tree)
R_60_co1_40_1_List <- foreach(i=1:length(species_List[[27]])) %do% sister(treeList$`/60_6_new`,species_List[[27]][i],type="terminal",label=T)
...
monotypic_genus_result <- genus_count(RefTree_R_co1_40_1_List,R_60_co1_40_1_List,7)
...
#Sister species for placement sequeneces(20%) after placing on bb tree(80% bb tree)
R_40_co1_60_1_List <- foreach(i=1:length(species_List[[17]])) %do% sister(treeList$`/40_6_new`,species_List[[17]][i],type="terminal",label=T)
...
RefTree_R_co1_80_1_List <- foreach(i=1:length(species_List[[7]])) %do% sister(ReferenceTree,species_List[[7]][i],type="terminal",label=T)
#Sister species for placement sequeneces(20%) after placing on bb tree(80% bb tree)
R_20_co1_80_1_List <- foreach(i=1:length(species_List[[7]])) %do% sister(treeList$`/20_6_new`,species_List[[7]][i],type="terminal",label=T)

#Call the function for bb40% and co1 60%
#For the N (order is,1= 20_1,2=20_10,3=20_2,4=20_3,5=20_4,6=20_5,7=20_6,8=20_7,9=20_8,10=20_9)
monotypic_genus_result <- genus_count(RefTree_R_co1_80_1_List,R_20_co1_80_1_List,7)
...
genus_count <- function(RefTree,placementTree,N){
  ...
  #genus.y col in unique_mismatches dataframe has the genus list of mismatches
  #Using that col and set diff function get the monotypic genus
  monotypic_genus <- setdiff(unique_mismatches$genus.y,Ref_Tree_distinct)
  monotypic_genus_count <- length(monotypic_genus)
  #Now let's see the number of monotypic placement genus with clade genus that all matches, zero matches and mixtures (close enough monotypic)
  match_monotypic <- mismatch_wt_names[mismatch_wt_names$genus.x==mismatch_wt_names$genus.y, ]
  
  mismatch_monotypic <- mismatch_wt_names[mismatch_wt_names$genus.x!=mismatch_wt_names$genus.y, ]
  #Let's see whats common in both (mixture set)
  #intersect of id
  mixtures <- intersect(match_monotypic$genus.y,mismatch_monotypic$genus.y)
  mixtures_count <- length(mixtures)
  #Only in match_monotypic
  All_match_monotypic <- setdiff(match_monotypic$genus.y,mismatch_monotypic$genus.y)
  All_match_monotypic_count <- length(All_match_monotypic)
  #Only in mismatch_monotypic
  
  All_mismatch_monotypic <- setdiff(mismatch_monotypic$genus.y,match_monotypic$genus.y)
  All_mismatch_monotypic_count <- length(All_mismatch_monotypic)
  results <- data.frame(genus_mismatches=uniq,unique_mismatches_count,monotypic_genus=monotypic_genus_count,mixed_monotypic=mixtures_count,All_match_monotypic=All_match_monotypic_count,All_mismatch_monotypic=All_mismatch_monotypic_count)
  return(results)
}
#Sister species of placement sequences in reference tree#for bb99%
RefTree_R_co1_1_1_List <- foreach(i=1:length(species_List[[47]])) %do% sister(ReferenceTree,species_List[[47]][i],type="terminal",label=T)
#Sister species for placement sequeneces(20%) after placing on bb tree(80% bb tree)
R_99_co1_1_1_List <- foreach(i=1:length(species_List[[47]])) %do% sister(treeList$`/99_6_new`,species_List[[47]][i],type="terminal",label=T)
...
monotypic_genus_result <- genus_count(RefTree_R_co1_1_1_List,R_99_co1_1_1_List,7)
